define([
    ...
    var gtd = function () {
        ...
        function createTab(isFirst) {
            ...
            var missions = getMissions().missions; // 用户任务数据
            var tabUl = $('#tab-ul'); // 分类列表ul
            var tabUlFragment = document.createDocumentFragment();
            var spanDeleteFolder = document.createElement('span'); // 删除图标
            var allMissionCount = 0; // 所有任务数
            var oAllMission = $('#all-mission'); // 显示所有任务的Dom
            spanDeleteFolder.className = 'delete-folder';

            /*进行递归append*/
            for (var i = 0, len = category.length; i < len; i++) {
                var deepth = 1;
                var oLi = recurTab(category[i], spanDeleteFolder, deepth, isFirst); // 递归调用返回tab-ul里面所有的li
        ...
        function cloneCategory(src, folderId) {
            ...
            if (srcClass === 'Object') {
                ...
                if (srcClass === 'Array') {
                    ...
                    if (srcClass === 'Number' || srcClass === 'String' || srcClass === 'Boolean' || srcClass === 'Date') {
                        return src;
                    }
                }
            }
        ...
        function getFolderById(folderId, categoryArr) {
            ...
            if (folderId === -1) {
                ...
                for (var i = 0, len = category.length; i < len; i++) {
                    ...
                    /*对应则返回*/
                    if (category[i].folderId === folderId) {
                        return category[i].children;
                    }
        ...
        function getEditingMission() {
            ...
            var parentId = oContMissionBody.dataset.parentId;
            var missionId = oContMissionBody.dataset.missionId;
        ...
        function _getMissionContElements() {
            ...
            var oContTitleBar = $('#cont-title-bar'); // 标题栏
            var oMissionTitle = oContTitleBar.getElementsByTagName('h2')[0]; // 标题
            var oMissionTime = $('#cont-mission-time span'); // 任务完成时间
            var oFinishBtn = $('#finish-mission');
            var oEditBtn = $('#edit-mission');
            var oSaveEdit = $('#save-edit');
            var oCancelEdit = $('#cancel-edit');
        ...
        function updateTimeline(type, isNewMission) {
            ...
            var aTypes = oMissionType.getElementsByTagName('a');
            var currentSelected;
            ...
            if (!currentTask || currentTask.tagName.toUpperCase() !== 'LI') {
                return;
            }
        ...
        function packTimeline(taskId, type) {
            ...
            for (var i = 0, len = missions.length; i < len; i++) {
                ...
                if (missions[i].parentId === taskId) {
                    ...
                    /*循环组装时间线li*/
                    for (var j = 0, childrenLen = children.length; j < childrenLen; j++) {
                        if (children[j].type !== type && type !== 'all') {
                            continue;
                        }
            ...
            /*如果这个task对应分类并没有任务*/
                ...
                if (!liFragment.childElementCount) {
                    var msgDiv = document.createElement('div');
        ...
        function switchMissionType() {
            var oMissionType = $('#mission-type');
            var aTypes = oMissionType.getElementsByTagName('a');
            var currentSelected = aTypes[0];
            var oTimelineOl = $('#mission-timeline ol'); // 获得时间线栏的ol
            var oMissionType = null;

            $.delegate('#mission-type', 'a', 'click', function (ev) {

                /*变更显示状态*/
                if (!$.hasClass(this, 'type-active')) {
                    currentSelected = $('#mission-type .type-active');
                }
                $.removeClass(currentSelected, 'type-active');
        ...
        function selectTask() {
            ...
            var missions = getMissions().missions; // 得到missions
            var oTimelineOl = $('#mission-timeline ol'); // 获得时间线栏的ol
            var currentSelected = $('#protected'); // 当前选择项
            var selectedId; // 当前选择项的id
            $.delegate('#tab-ul', 'li', 'click', function (ev) {
                ...
                if (!$.hasClass(this, 'tab-tree-li') && !$.hasClass(this, 'font-active')) {
                    ...
                    /*如果已有选中的，取消它的active*/
                    if (currentSelected) {
                        $.removeClass(currentSelected, 'font-active');
                    }
        ...
        function selectMission() {
            ...
            $.delegate('#mission-timeline ol', 'dd', 'click', function (ev) {
                ...
                if (!$.hasClass(this, 'font-active')) {
                    ...
                    if (ocurrentSelected) {
                        $.removeClass(ocurrentSelected, 'font-active');
                    }
        ...
        function saveEdit() {
            $.on('#save-edit', 'click', function () {
                ...
                /*获取dom元素集合*/
                var contElements = getMissionContElements();
                var oEditArea = $('#cont-mission-edit'); // 任务内容编辑框
                var oEditTitle = $('#cont-title-edit'); // 标题编辑框
                var oEditTime = $('#cont-time-edit'); // 任务完成时间编辑框
                var mission = getEditingMission(); // 当前编辑中的任务
        ...
        function cancelEdit() {
            $.on('#cancel-edit', 'click', function () {
                ...
                var mission = getEditingMission(); // 当前编辑中的任务model
                if ($.hasClass(contElements.oContMissionBody, 'mission-editing')) {
                    if (!confirm('正在编辑中，要放弃保存吗')) {
                        return;
                    }
        ...
        function deleteFolder() {
            $.delegate('#tab-ul', 'span', 'click', function () {
                ...
                if ($.hasClass(this, 'delete-folder') && confirm('确定要删除这个文件夹吗')) {
                    ...
                    var folderId = parseInt(currentFolder.dataset.folderId);
            ...
            $.on('#add-tab', 'click', function () {
                ...
                while (oActiveTab.tagName.toUpperCase() !== 'DIV') {
                    ...
                    for (var i = 0, len = childrenElm.length; i < len; i++) {
                        if (childrenElm[i].tagName.toUpperCase() === 'DIV') {
                            oActiveTab = childrenElm[i];
                        }
                    }
                }
